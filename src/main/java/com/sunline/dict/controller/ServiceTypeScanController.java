 ...
 */
@RestController
@RequestMapping("/api/service-type-scan")
@CrossOrigin
public class ServiceTypeScanController {
    
    private static final Logger log = LoggerFactory.getLogger(ServiceTypeScanController.class);
    
    @Autowired
    private ServiceTypeScanService serviceTypeScanService;
    
    /**
     * 开始扫描ServiceType文件（文件上传方式）
     */
    @PostMapping("/scan")
    public Result<Map<String, Object>> scanServiceTypeFiles(
        ...
        try {
            ...
            log.info("开始扫描ServiceType文件，文件：{}，jar包：{}", originalFilename, jarNames);
            
            // 在Controller层（同步）先保存文件，避免异步时临时文件被清理
            File savedFile = saveUploadedFile(jarFile, "service-type-upload");
            
            // 异步执行扫描（传递文件路径）
            serviceTypeScanService.scanServiceTypeFiles(savedFile.getAbsolutePath(), jarNames);
    ...
    public Result<Map<String, Object>> getScanProgress() {
        try {
            Map<String, Object> progress = serviceTypeScanService.getScanProgress();
    ...
    public Result<String> cancelScan() {
        try {
            serviceTypeScanService.cancelScan();
